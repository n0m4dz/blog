...
// Imports
document.write('<script src="components/imagesloaded/imagesloaded.pkgd.min.js"></script>');
...
(function($) {
	...
	$.humanTimeDiff = function(date) {
		...
		var time_diff = {
			months:  ~~(delta / 2626560), // ~30.4 days per month
			days:    ~~(delta / 86400),
			hours:   ~~(delta / 3600),
			minutes: ~~(delta / 60),
			seconds: ~~(delta)
		...
		if (typeof $.humanTimeDiff.i18n == 'undefined') {
	...
	$.fn.countdown = function() {
		...
		init = function() {
			...
			items = el
				.filter(function() {
					return ['object', 'string'].indexOf(typeof $(this).data('countdown-date')) >= 0;
				})
				.each(function() {
					var
						item = $(this),
					...
					if (typeof date == 'string') {
						item.data('countdown-date', new Date(date));
					}
				});
	...
	$.fn.bricks = function(options) {
		...
		el.refresh = function() {
			...
			_item.each(function() {
				...
				item
					...
					.data({
						...
						'bricks-height': el.options.precise ? item[0].getBoundingClientRect().height : item.outerHeight()
					});
			});
			...
			// Container height
			items.css('height', Math.max.apply(null, stack));

			// Parent element
			el.parent().closest('.bricks').bricks('refresh');
				...
				success: function(data) {
					...
					posts.imagesLoaded(function() {
						...
						var
							...
							height = last.position().top + last.outerHeight(true),
